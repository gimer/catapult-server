...
#define SHA3_TRAITS_BASED_TEST(TEST_NAME) \
	...
	TEST(Kmac_256, SampleTestVector_4) {
		...
		// Act:
		Kmac256(key, data, output, "My Tagged Application");
	...
	TEST(Kmac_256, ThrowsWhenOutputBufferSizeIsZero) {
		...
		// Act + Assert:
		EXPECT_THROW(Kmac256(key, data, { output.data(), 0 }, "My Tagged Application"), catapult_invalid_argument);
	}
	...
	TEST(Kmac_256, ThrowsWhenNumberOfRepetitionsIsTooLarge) {
		...
		EXPECT_THROW(Kmac256(key, data, { output.data(), largeSize }, "My Tagged Application"), catapult_invalid_argument);
	}
